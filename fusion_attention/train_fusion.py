...
for run_count in range(1, num_models_to_train + 1):
    ...
    run_dir = create_run_directory(run_count)
    ...
    with open(log_file, 'w') as f:
        ...
        f.write(f"Run Number: {run_count}\n")
    ...
    for epoch in range(num_epochs):
        ...
        print(f'Run {run_count} - Epoch [{epoch+1}/{num_epochs}], Loss: {avg_loss:.4f}, Val Loss: {val_loss:.4f}, Time: {epoch_time:.2f}s, GPU Memory: {gpu_memory:.2f} MB')
        ...
        if val_loss < best_val_loss:
            ...
            print(f'Saving best model for run {run_count} with validation loss: {best_val_loss:.4f}')
        else:
            epochs_no_improve += 1
    ...
    # Calculate total training time for the current run
    ...
    print(f"Training complete for run {run_count}. Best model and final model weights are saved.")
