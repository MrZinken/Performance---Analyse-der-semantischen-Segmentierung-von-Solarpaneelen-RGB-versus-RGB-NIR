        ...
        for ann in image_annotations:
            ...
            if np.any(surrounding_mask):
                for channel, name in enumerate(['red', 'green', 'blue', 'nir']):
                    ...
                    surrounding_mean_brightness = np.mean(img_data[surrounding_mask, channel])
                    overall_brightness_surrounding[name].append(surrounding_mean_brightness)

                    # Difference in brightness
                    brightness_diff = panel_mean_brightness - surrounding_mean_brightness
                    ...
                    surrounding_homogeneity = np.std(img_data[surrounding_mask, channel])
                    overall_homogeneity_surrounding[name].append(surrounding_homogeneity)
...
for name in ['red', 'green', 'blue', 'nir']:
    ...
    print(f"{name.capitalize()} Channel - Average Panel Brightness: {avg_panel_brightness:.4f}")
    print(f"{name.capitalize()} Channel - Average Surrounding Brightness: {avg_surrounding_brightness:.4f}")
    print(f"{name.capitalize()} Channel - Average Brightness Difference (Panel - Surrounding): {avg_brightness_diff:.4f}")
    print(f"{name.capitalize()} Channel - Average Panel Homogeneity (StdDev): {avg_panel_homogeneity:.4f}")
    print(f"{name.capitalize()} Channel - Average Surrounding Homogeneity (StdDev): {avg_surrounding_homogeneity:.4f}")
    print(f"{name.capitalize()} Channel - Average Homogeneity Difference (Panel - Surrounding): {avg_homogeneity_diff:.4f}")
    print('-' * 50)
