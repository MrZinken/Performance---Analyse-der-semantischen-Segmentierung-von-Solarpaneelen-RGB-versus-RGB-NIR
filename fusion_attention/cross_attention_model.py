...
# ResNet50 Backbone with batch normalization for either 3 or 4 channels
class ResNet50Backbone(nn.Module):
    def __init__(self, input_channels: int = 3, pretrained_weights: str = 'imagenet', nir_init_method: str = 'red'):
    ...
    def initialize_nir_with_red(self):
        with torch.no_grad():
            self.backbone.conv1.weight.data[:, 3:4, :, :] = self.backbone.conv1.weight.data[:, 0:1, :, :]
...
class MultiHeadSelfAttention(nn.Module):
    ...
    def forward(self, rgb_features: torch.Tensor, nir_features: torch.Tensor) -> torch.Tensor:
        ...
        query = self.query_conv(rgb_features).view(batch_size, self.num_heads, self.head_dim, -1)
        key = self.key_conv(nir_features).view(batch_size, self.num_heads, self.head_dim, -1)
        value = self.value_conv(nir_features).view(batch_size, self.num_heads, self.head_dim, -1)

        # Compute attention weights
        attention_scores = torch.matmul(query.permute(0, 1, 3, 2), key) / (self.head_dim ** 0.5)
...
class FusionAttention(nn.Module):
    ...
    def forward(self, rgb_features: torch.Tensor, nir_features: torch.Tensor) -> torch.Tensor:
        if self.fusion_type == 'multihead_attention':
            ...
            raise ValueError("Invalid fusion type. Choose 'multihead_attention' or 'late_fusion'.")
...
class MultimodalSegmentationModel(nn.Module):
    def __init__(self, num_classes: int = 2, nir_init_method: str = "red", fusion_type: str = 'multihead_attention'):
        super(MultimodalSegmentationModel, self).__init__()
        # Initialize RGB and NIR backbones
        self.rgb_backbone = ResNet50Backbone(input_channels=3, pretrained_weights='imagenet')
        self.nir_backbone = ResNet50Backbone(input_channels=1, pretrained_weights=None, nir_init_method=nir_init_method)
    ...
    def forward(self, fused_image: torch.Tensor) -> torch.Tensor:
        ...
        return F.interpolate(output, size=(fused_image.shape[2], fused_image.shape[3]), mode='bilinear', align_corners=False)
