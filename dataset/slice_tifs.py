...
def slice_image_and_save(input_image_path, output_dir, slice_size=1000, max_slices=100):
    ...
    if image_array.shape[2] != 4:
        raise ValueError(
            f"Image {input_image_path} does not have 4 channels (RGB + IR). Found: {image_array.shape[2]} channels."
        )
...
def process_all_tiffs_in_folder(
    input_folder, output_dir, slice_size=1000, max_slices=100
):
